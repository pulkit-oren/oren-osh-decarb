...
describe("scope 2 tabs render", () => {
  for (const [name, Tab] of cases) {
    it(`${name} renders with seeded defaults`, () => {
      const html = renderToString(
        // ScenarioProvider included: the Compare tab hosts the combined S1+S2 pathway
        React.createElement(ScenarioProvider, null,
  ...
  it("BuilderTab home shows facility boxes, mode tabs, and results panel", () => {
    const html = renderToString(
    ...
    // Mode tabs (Balance to target moved up to the cross-scope BuilderHub)
